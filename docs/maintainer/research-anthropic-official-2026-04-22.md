...
> **Status update (2026-04-23, Phase κ-2)**: 本節は 2026-04-22 時点の初期調査スナップショット
> + Phase η P0-κ / Phase κ-2 完了状況の追記の合成。2026-04-23 Phase κ-2 で `WorktreeCreate`
> blocking protocol と `WorktreeRemove` observability の登録が完了し、`HookResult` の
> universal field (`continue` / `stopReason` / `suppressOutput`) および
> `worktreePath` (WorktreeCreate 専用 top-level bridge) が追加されている。

`hooks/hooks.json` 登録 event (Phase κ-2 完了後、2026-04-23):
...
Stop
TaskCompleted
WorktreeCreate    ← Phase κ-2 (blocking protocol production)
WorktreeRemove    ← Phase η P0-κ (non-blocking observability)
```

網羅率 (2026-04-23 更新):

```text
covered=PreToolUse, PostToolUse, PermissionRequest, SessionStart, SessionEnd, PreCompact, SubagentStop, TaskCreated, Stop, TaskCompleted, WorktreeCreate, WorktreeRemove (12 / 27)
missing=InstructionsLoaded, UserPromptSubmit, UserPromptExpansion, PermissionDenied, PostToolUseFailure, Notification, SubagentStart, StopFailure, TeammateIdle, ConfigChange, CwdChanged, FileChanged, PostCompact, Elicitation, ElicitationResult (15 / 27)
```

実装の実態 (2026-04-23 更新):

- `hooks.json` は `SessionStart` / `SessionEnd` を登録している
- しかし `core/src/index.ts` では `session-start` / `session-end` は単に `{ decision: "approve" }` を返すだけで no-op
- hook runtime は command hook のみ (http / prompt / agent / async は未対応)
- `scripts/hook-dispatcher.mjs` は fail-open で常に `{"decision":"approve"}` を返す (WorktreeCreate 失敗経路のみ exit 1 で blocking 失敗を伝搬)
- internal `HookResult` 型は `decision`, `reason`, `systemMessage` に加え、Phase κ-2 で `worktreePath`, `continue`, `stopReason`, `suppressOutput` を optional として追加 (後方互換、command hook raw stdout mode で使用)

### 3.3 ギャップ分析

#### イベントカバレッジ

- 27 event 中 12 event を登録 (Phase κ-2 後)
...
- Phase η 残 P0 (Phase η 残作業、backlog #2): `PostToolUseFailure`, `UserPromptSubmit`

#### 実装 fidelity

- 公式 JSON output の `continue`, `stopReason`, `suppressOutput`, `systemMessage` は harness `HookResult` でサポート済 (Phase κ-2)
- event-specific outputs (`updatedPermissions`, `updatedInput`, `retry`, `watchPaths`, `updatedMCPToolOutput`, `action`, `content`) は未実装 (command hook raw stdout 運用に集中、HTTP hook サポート時に `hookSpecificOutput` nested pattern で追加予定、Phase θ)
- `worktreePath` は Phase κ-2 で top-level optional として HookResult に載せ、`index.ts main()` の worktree-create 分岐で raw stdout に書き出し (公式 command hook protocol に整合)
