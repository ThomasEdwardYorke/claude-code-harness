...
2. What's the migration cutoff version? → Revised (as of v0.3.0 release, 2026-04-23): complete Phase 1 by v0.2.0 (DONE), Phase 2 by v0.4.0, Phase 3 by v0.5.0, Phase 4 CI-enforced from v0.4.0. Phase 1 compliance applies in v0.2.0 and v0.3.0 (no new Japanese shipped spec introduced); Phase 2 critical-path migration (harness-work / tdd-implement / parallel-worktree / worker / reviewer) remains deferred to v0.4.0. v0.3.0 is a minor-feature release (WorktreeCreate blocking protocol + .coderabbit.yaml repo config), not a migration milestone.
