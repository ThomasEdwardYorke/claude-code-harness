...
- **Route-level integration tests for both hooks** (`index.test.ts`, 3 tests) and **handler unit tests** (`worktree-lifecycle.test.ts`, new file, 13 tests including empty-string and newline-injection edge cases). New `it` / `it.each` blocks added in this increment total 21 (net, across 3 test files). The project-wide vitest case count (which expands `it.each` parametrisations from many earlier test files) moved from 1685 to ~1757 after this commit.
