...
describe("plugin.json component 宣言 (Anthropic 公式仕様: 明示宣言でデフォルトパス変更耐性)", () => {
  ...
  // 標準 path (`hooks/hooks.json`) は omit する。他 field (commands / agents) の
  // explicit array 列挙は Phase μ forcing function と連動するため本 rule の scope 外。
  //
  // path normalization: `./hooks/./hooks.json` / `hooks/../hooks/hooks.json` 等の
  // 同一ファイルを指す variant も wrap できるよう `pathPosix.normalize` 経由で比較する
  // (Codex adversarial review PII-3)。plugin.json は JSON 形式で常に forward-slash
  // 規約のため POSIX normalize で十分 (Windows back-slash variant は manifest 内で
  // そもそも不正)。
