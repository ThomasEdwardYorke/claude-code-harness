  ...
  const securityPatterns: Array<{ pattern: RegExp; message: string }> = [
    {
      ...
      message:
        "Embedding a secret from process.env directly into a string literal — consider redaction.",
    },
    {
      pattern: /eval\s*\(\s*(?:request|req|input|param|query)/i,
      message:
        "Passing user input to eval() — arbitrary code execution risk (RCE).",
    },
    {
      pattern: /exec\s*\(\s*`[^`]*\$\{/,
      message:
        "Passing a template literal to exec() — command injection risk.",
    },
    {
      pattern: /innerHTML\s*=\s*(?:.*\+.*|`[^`]*\$\{)/,
      message:
        "Assigning interpolated strings to innerHTML — XSS risk.",
    },
    {
      pattern:
        /(?:password|passwd|secret|api_key|apikey)\s*=\s*["'][^"']{8,}["']/i,
      message:
        "Hardcoded secret (password / API key) detected.",
    },
  ];
