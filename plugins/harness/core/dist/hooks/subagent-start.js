...
export async function handleSubagentStart(input, options) {
    ...
    const agentTypeKey = typeof input.agent_type === "string" && input.agent_type.length > 0
        ? input.agent_type
        : "unknown";
    ...
    if (utf8ByteLength(additionalContext) > cfg.maxTotalBytes) {
        if (cfg.fenceContext && lines.length >= 3) {
            ...
            const overhead = utf8ByteLength(openFence) + utf8ByteLength(closeFence) + 2;
            const markerSuffix = `\n[harness ${nonce}] context truncated at ${cfg.maxTotalBytes} bytes`;
            const bodyBudget = cfg.maxTotalBytes - overhead - utf8ByteLength(markerSuffix);
            let truncatedBody;
            if (bodyBudget > 0) {
            ...
            const sliceBudget = Math.max(0, cfg.maxTotalBytes - utf8ByteLength(truncationMarker));
            additionalContext =
