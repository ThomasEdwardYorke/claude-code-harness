 ...
 */
function replicateWorktreeInclude(repoRoot, worktreePath) {
    const notes = [];
    ...
    try {
        ...
        if (stat.size > MAX_PLANS_SIZE) {
            notes.push("[worktreeinclude-skipped] .worktreeinclude exceeds safe size cap; replication aborted");
            return notes;
        }
    ...
    for (const entry of entries) {
        ...
        if (entry.includes("..") || entry.startsWith("/") || entry.startsWith("\\")) {
            notes.push(`[worktreeinclude-skipped] unsafe path rejected: ${sanitizeForContext(entry)}`);
            continue;
        }
        ...
        try {
            ...
            notes.push(`[worktreeinclude-skipped] failed to resolve src: ${sanitizeForContext(entry)}`);
            continue;
        }
        ...
        if (!srcNorm.startsWith(repoNorm + sep) && srcNorm !== repoNorm) {
            notes.push(`[worktreeinclude-skipped] src escaped repo root: ${sanitizeForContext(entry)}`);
            continue;
        }
        if (!dstNorm.startsWith(wtNorm + sep) && dstNorm !== wtNorm) {
            notes.push(`[worktreeinclude-skipped] dst escaped worktree: ${sanitizeForContext(entry)}`);
            continue;
        }
        if (!existsSync(src)) {
            notes.push(`[worktreeinclude-skipped] src missing: ${sanitizeForContext(entry)}`);
            continue;
        }
        try {
            ...
            notes.push(`[worktreeinclude-failed] ${sanitizeForContext(entry)}: ${sanitizeForContext(msg)}`);
        }
    }
    return notes;
}
export async function handleWorktreeCreate(input) {
    ...
    if (repoRoot === null) {
        ...
        const reason = "failed to resolve repo top-level (git rev-parse --show-toplevel)";
        sections.push(`[error] ${reason}`);
