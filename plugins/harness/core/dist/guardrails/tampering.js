...
export const TAMPERING_PATTERNS = [
    // --- Skipped tests ---
    {
        id: "T01:it-skip",
...
export function detectTampering(text, isTest) {
    const warnings = [];
 ...
 */
export function detectTestTampering(input, config) {
    ...
    const warningLines = warnings
        ...
        .join("\n");
    const message = `[harness] Tampering patterns detected in ${fileType} \`${filePath}\`:\n\n` +
        warningLines +
