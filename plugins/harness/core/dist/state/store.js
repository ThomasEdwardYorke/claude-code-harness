...
export class HarnessStore {
    ...
    upsertSession(session) {
        ...
        const existing = this.state.sessions.findIndex((s) => s.session_id === session.session_id);
        const row = {
    ...
    receiveSignals(sessionId) {
        const matches = this.state.signals.filter((s) => s.consumed === 0 &&
    ...
    setWorkState(sessionId, options = {}) {
        ...
        const existing = this.state.work_states.findIndex((s) => s.session_id === sessionId);
        if (existing >= 0) {
    ...
    getWorkState(sessionId) {
        ...
        const row = this.state.work_states.find((s) => s.session_id === sessionId && s.expires_at > now);
        if (row === undefined)
            return null;
        return {
    ...
    cleanExpiredWorkStates() {
        ...
        this.state.work_states = this.state.work_states.filter((s) => s.expires_at > now);
