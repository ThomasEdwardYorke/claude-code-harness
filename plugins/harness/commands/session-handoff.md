...
| S-13 | `backlog.md` が肥大化 — 150 行超で WARN / 200 行超で FAIL (**再肥大化 guard**、monolithic session-prompt 再発防止) | `wc -l` | WARN (150+) / FAIL (200+) |
...
300 行超で S-12/S-13 が分割検討シグナル。report 要約外の詳細 (Quick-start bash
全文、運用ルール、背景 docs 等) も **Claude context に ingest 済**なので、
check 後の再 Read は不要 (Anti-pattern #10)。
