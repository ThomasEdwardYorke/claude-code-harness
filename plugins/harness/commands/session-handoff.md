...
├── <project>-backlog.md          # 優先度付き pending list (dispatchable view)
├── <project>-roadmap.md          # **NEW (optional, handoff mode)**: Phase / Week / AC 源泉
├── <project>-design-decisions.md # append-only 恒久方針
├── <project>-phases.md           # ロードマップ / Phase 定義 (optional, legacy)
...
**4 層構成 (handoff mode、`harness.config.json` の `work.taskTrackerMode = "handoff"` で opt-in)**:

| 層 | ファイル | 役割 |
|---|---|---|
| 1 | `<project>-current.md` | 鳥瞰図 — 次セッションが最初に読む 1 ファイル |
| 2 | `<project>-backlog.md` | dispatchable view — `/harness-work` がここからタスクを取り出す |
| 3 | `<project>-roadmap.md` | source-of-truth — Phase / Week / Task の AC, depends_on, estimated を保持 |
| 4 | `<project>-design-decisions.md` | append-only — 恒久方針 |

`harness.config.json` で次のように指定する:
...
未設定 (デフォルト `taskTrackerMode = "plans"`) の場合は従来の `Plans.md` 経路が継続。
`handoffPaths` の 4 キー全てが必須で、欠けた場合は loader が stderr 警告を出して
`plans` モードへ silent fallback する (`/harness-work` が誤って未定義参照しないため)。
...
**Plans-mode (legacy)**: 1 行 list 形式

```markdown
- [High|Med|Low] <Phase 識別子>: <1 行説明> (archive: session-YYYY-MM-DD-<slug>.md)
```

**Handoff-mode (`taskTrackerMode = "handoff"`)**: heading + optional YAML metadata
...
roadmap_ref: <pointer into roadmap.md, e.g. phase-1.week-2>
worktree: <relative path to worktree, optional>
pr: <PR reference, optional>
```
```

heading の `[Priority]` / `<id>` / `<title>` がデフォルトで、直後の fenced YAML block
が機械可読メタデータ (override) になる。`/harness-work` の handoff モードはこの
構造を `parseBacklog()` (`plugins/harness/core/src/work/backlog-parser.ts`) で読み、
`status: pending` のものを優先度順に dispatch する。

優先度ラベル (Critical/High/Med/Low) と任意の archive 参照を必ず付ける。
archive への逆参照で「なぜこの申送が発生したか」を辿れるようにする。

### Roadmap Layer (handoff mode、`<project>-roadmap.md`)

Phase / Week / Task の階層構造で AC, 依存関係, 見積を保持する source-of-truth。
`backlog.md` は dispatchable view (今すぐ着手すべきもの) なのに対し、`roadmap.md`
は計画全体の不変表現 (このプロジェクトが何を目指していて、何が残っているか)。
...
```yaml
id: <stable id matching backlog entry id>
phase: 1
week: 1
priority: Critical|High|Med|Low
...
<Task の AC / 設計メモ / 関連 issue 等を本文に記述>
```

**フィールド最小例 (Task heading 直後の YAML、3-5 行で済むケース)**:

```yaml
id: phase-1.week-1.task-9f
phase: 1
week: 1
priority: High
labels: [feature]
...
`backlog.md` の各 entry は `roadmap_ref` で `roadmap.md` の Task の `id` を指す。
Roadmap は読み取り中心 (heading 階層を変更すると Phase/Week 番号が破壊されるため
慎重に)、Backlog は in/out が活発 (タスク完了で entry 削除、新申送で entry 追加)。
...
| S-14 | **handoff mode**: `handoffPaths.roadmap` が指す `roadmap.md` が存在しない (Gate 4) | `harness.config.json` 読込 → `Glob` 確認 | FAIL |
| S-15 | **handoff mode**: backlog entry の `roadmap_ref` が `roadmap.md` 内に存在しない (Gate 4) | `parseBacklog()` 結果と roadmap heading 突合 | WARN |
| S-16 | **migration in progress**: `Plans.md` と handoff/ が両方存在 (Gate 4) | 両 file の `existsSync` | INFO |
| S-17 | **handoff mode**: `roadmap.md` が 30 日以上 commit なし + backlog に新 entry あり (Gate 4) | `git log -1` | INFO |

#### Gate 4 — Roadmap Freshness (handoff mode のみ、v3 新設)

**実行条件**: `harness.config.json` の `work.taskTrackerMode === "handoff"` かつ
`work.handoffPaths.roadmap` が設定されている時のみ起動する。Plans-mode (default)
プロジェクトでは Gate 4 全体を SKIP し、output template でも block 全体を省略
(従来運用と完全互換)。

**目的**: 4 層構成の source-of-truth (`roadmap.md`) と dispatchable view
(`backlog.md`) の整合性を確認し、roadmap が陳腐化していないか監視する。
具体的には S-14 〜 S-17 を走査:

- **S-14 (FAIL)**: `handoffPaths.roadmap` が指す `roadmap.md` が存在しない →
  handoff mode は roadmap layer を前提にしているため、欠如は致命的。
  Recommended Remediation で `init` または手動作成を案内。
- **S-15 (WARN)**: `parseBacklog()` で取得した `BacklogEntry.roadmapRef` が
  `roadmap.md` 内のいずれの Task heading id ともマッチしない → backlog entry が
  孤立しているか、roadmap が古い。両 file を見直す案内。
- **S-16 (INFO)**: `Plans.md` と handoff/ ディレクトリが両方存在 → 移行作業が
  進行中であることの観察的シグナル。verdict に影響せず、moderator の気づき促進。
- **S-17 (INFO)**: `roadmap.md` の最終 commit が 30 日以上前 + `backlog.md` に
  新規 entry 追加あり → roadmap が pending 状態を反映していない可能性。

**Gate 4 の git 不可環境ハンドリング**: S-14 / S-15 / S-16 は filesystem 操作
(existsSync / Glob) のみで実行可能、git 非依存。S-17 のみ `git log` 依存
なので Edge Case (git unavailable) では SKIP し、output に明示。
...
**Gate 4 — Roadmap Freshness** (handoff mode のみ — Plans-mode では block 全体を省略)

- **Mode**: handoff (`taskTrackerMode = "handoff"`)
- **Roadmap path**: `<handoffPaths.roadmap>`
- **Roadmap freshness signals**:
  - ✅ S-14: `roadmap.md` 存在
  - ⚠️ S-15: backlog entry `<id>` の `roadmap_ref=<ref>` が roadmap.md に未存在
  - ℹ️ S-16: `Plans.md` も同居 (移行進行中)
  - ℹ️ S-17: `roadmap.md` 30 日 commit なし + 新 backlog entry あり
...
- [ ] **Gate 1-3 を全て実行**し、Structural → Content → Synthesis の順で
      ...
      限り情報取得を継続する)
- [ ] **Gate 4 (handoff mode のみ)**: `harness.config.json` の
      `work.taskTrackerMode === "handoff"` かつ `work.handoffPaths.roadmap` が
      設定されている時にのみ起動。Plans-mode では block 全体 SKIP し、output
      template でも省略 (従来運用と完全互換)
